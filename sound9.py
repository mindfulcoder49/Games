...
# Smoothing factors for different frequency bands
SMOOTHING_FACTORS = {
    'bass': 0.9,
    'mid': 0.9,
    'treble': 0.9
...
# Configurable fields
config_fields = ["LOWER_FREQ", "UPPER_FREQ", "THRESHOLD", "MAX_CIRCLE_SIZE"]
config_values = [LOWER_FREQ, UPPER_FREQ, THRESHOLD, MAX_CIRCLE_SIZE]
current_field = 0
input_value = ""
...
# Find the index of the loopback device
device_info = sd.query_devices()
loopback_device = None
for idx, device in enumerate(device_info):
    if 'pipewire' in device['name'].lower() or 'stereo mix' in device['name'].lower():
        loopback_device = idx
        break

if loopback_device is None:
    print("No loopback device found. Please ensure your system has a loopback device enabled.")
    sys.exit(1)

# Start audio stream
audio_data = np.zeros(block_size)
stream = sd.InputStream(callback=audio_callback, device=4, channels=1, samplerate=sample_rate, blocksize=block_size)
stream.start()

# Function to apply exponential moving average smoothing to specific bands
def smooth_data(data, prev_smooth_data, smoothing_factors, frequency_band_indices):
    smoothed_data = np.zeros_like(data)
    for band, indices in frequency_band_indices.items():
        factor = smoothing_factors[band]
        smoothed_data[indices] = factor * prev_smooth_data[indices] + (1 - factor) * data[indices]
    return smoothed_data

# Initialize smoothed frequency magnitudes for different bands
prev_smoothed_magnitudes = np.zeros(block_size // 2)

# Define frequency band indices
frequency_band_indices = {
    'bass': np.where((np.fft.fftfreq(block_size, 1/sample_rate)[:block_size // 2] < 250))[0],
    'mid': np.where((np.fft.fftfreq(block_size, 1/sample_rate)[:block_size // 2] >= 250) & 
                    (np.fft.fftfreq(block_size, 1/sample_rate)[:block_size // 2] < 2000))[0],
    'treble': np.where((np.fft.fftfreq(block_size, 1/sample_rate)[:block_size // 2] >= 2000))[0]
}
...
def frequency_to_note_name(frequency):
    if frequency == 0:
        return None
    note_number = 48 * np.log2(frequency / A4_FREQ)
    note_index = int(round(note_number)) % 24
    note_names = [
...
while True:
    ...
    LOWER_FREQ, UPPER_FREQ, THRESHOLD, MAX_CIRCLE_SIZE = config_values
    ...
    # Smooth frequency magnitudes for each band
    smoothed_magnitudes = smooth_data(freq_magnitudes, prev_smoothed_magnitudes, SMOOTHING_FACTORS, frequency_band_indices)
