...
HOP_SIZE = 512  # Number of samples to shift for each FFT calculation
...
# Start audio stream
audio_data = []
stream = sd.InputStream(callback=audio_callback, channels=1, samplerate=sample_rate, blocksize=HOP_SIZE)
stream.start()
...
while True:
    ...
    if len(audio_data) >= WINDOW_SIZE:
        # Get the current audio window and remove it from the buffer
        window = np.array(audio_data[:WINDOW_SIZE])
        audio_data = audio_data[HOP_SIZE:]

        # Compute FFT and get frequency magnitudes
        fft_data = np.fft.fft(window * np.hanning(WINDOW_SIZE))
            ...
            if magnitude > THRESHOLD:  # Configurable threshold for circle display
                ...
                if LOWER_FREQ <= frequency <= UPPER_FREQ:  # Only display circles within frequency range
                    ...
                    if note_name:
                        ...
                        # Display the note name in the middle of the circle
                        font = pygame.font.SysFont(None, circle_radius // 2)
